...
class AdminDeleteRole extends React.Component {
constructor(props){
  super(props);
  this.state = {
    roleId: "",
...
handleClickAddRole(event){
  this.setState ({
    ...
    if(Object.keys(this.state.errors).length > 0){
      ...
      AdminService.deleteUserRoles(this.state.roleId, this.state.userId)
      .then(response => {
        this.setState({
          roleId: "",
          userId: "",
...
render() {
  return(
    <div className = 'admin-content-holder flex-column' >
      <a className = 'subheader-content-profile'>Wycofywanie ról</a>
      <div className = 'flex-column width-100'>
        <form className = 'edit-user-form flex-column'>
          ...
          <br/><br/>
          <div>
            <a className = 'button-green' onClick = {this.handleClickAddRole}>Wycofaj</a>
          </div>
        </form>
      </div>
...
export default AdminDeleteRole;
