...
it('renders', () => {
  shallow(<DoctorPublicItem />);
});

it('accepts props', () => {
  const wrapper = mount(<DoctorPublicItem name = {user.name} id = {user.id} clickCallback = {clickCallback}/>);
...
describe("Component functionality", () => {
  ...
  it('displays name', () => {
    const wrapper = mount(<DoctorPublicItem name = {user.name} id = {user.id} />);
  ...
  it('firesButton',  () => {
    ...
    const { getByRole } = render(<DoctorPublicItem name = {user.name} id = {user.id} clickCallback = {mockedCallBack}/>);
